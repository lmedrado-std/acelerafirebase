    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{currentSeller.ticketAverage.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</div>
             <p className="text-xs text-muted-foreground">Progresso para a próxima meta</p>
            <Progress value={getGoalProgress(currentSeller.ticketAverage, 'ticketAverage')} className="mt-2 h-2" />
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Produtos por Atendimento (PA)</CardTitle>
            <Box className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{currentSeller.pa.toFixed(1)}</div>
             <p className="text-xs text-muted-foreground">Progresso para a próxima meta</p>
            <Progress value={getGoalProgress(currentSeller.pa, 'pa')} className="mt-2 h-2" />
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Participe desses desafios para ganhar pontos extras!</CardDescription>
        </CardHeader>
        <CardContent>
